...
use Cavidel\ArsLedger6;
use Cavidel\ArsBank6;
use DB;

class jsonResponseController6 extends Controller
{
    /*
    |-----------------------------------
    | Add Ledger Item
    ...
    public function loadAllItems(Request $request)
    {
        ...
        $all_bank_items = DB::table('ars_banks6')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
        ...
        $all_ledger_items = DB::table('ars_ledgers6')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank6::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank6::find($bank->id);
            ...
            $ledger = ArsLedger6::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger6::find($ledger->id);
