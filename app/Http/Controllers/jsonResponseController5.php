...
use Cavidel\ArsLedger5;
use Cavidel\ArsBank5;
use DB;
...
class jsonResponseController5 extends Controller
{
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank5::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank5::find($bank->id);
            ...
            $ledger = ArsLedger5::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger5::find($ledger->id);
    ...
    public function saveDataForSorting(Request $request)
    {
      ...
      $check_data = DB::table('tblReconSetup5')->get();
      if (count($check_data) > 0) {
          $proccess_update = DB::table('tblReconSetup5')->update([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
          ...
          $proccess_update = DB::table('tblReconSetup5')->insert([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
