...
class jsonResponseController7 extends Controller
{
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank7::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank7::find($bank->id);
            ...
            $ledger = ArsLedger7::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger7::find($ledger->id);
    ...
    public function saveDataForSorting(Request $request)
    {
      ...
      $check_data = DB::table('tblReconSetup7')->get();
      if (count($check_data) > 0) {
          $proccess_update = DB::table('tblReconSetup7')->update([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
          ...
          $proccess_update = DB::table('tblReconSetup7')->insert([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
