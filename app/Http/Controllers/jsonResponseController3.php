...
class jsonResponseController3 extends Controller
{
    ...
    public function loadAllItems(Request $request)
    {
        ...
        $all_bank_items = DB::table('ars_banks3')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
        ...
        $all_ledger_items = DB::table('ars_ledgers3')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
    ...
    public function saveDataForSorting(Request $request)
    {
      ...
      $check_data = DB::table('tblReconSetup3')->get();
      if (count($check_data) > 0) {
          $proccess_update = DB::table('tblReconSetup3')->update([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
          ...
          $proccess_update = DB::table('tblReconSetup3')->insert([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
