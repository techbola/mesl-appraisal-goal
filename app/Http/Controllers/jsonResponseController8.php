...
use Cavidel\ArsLedger8;
use Cavidel\ArsBank8;
use DB;
...
class jsonResponseController8 extends Controller
{
    ...
    public function loadAllItems(Request $request)
    {
        ...
        $all_bank_items = DB::table('ars_banks8')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
        ...
        $all_ledger_items = DB::table('ars_ledgers8')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank8::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank8::find($bank->id);
            ...
            $ledger = ArsLedger8::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger8::find($ledger->id);
    ...
    public function saveDataForSorting(Request $request)
    {
      ...
      $check_data = DB::table('tblReconSetup8')->get();
      if (count($check_data) > 0) {
          $proccess_update = DB::table('tblReconSetup8')->update([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
          ...
          $proccess_update = DB::table('tblReconSetup8')->insert([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
