...
use Cavidel\ArsLedger9;
use Cavidel\ArsBank9;
use DB;

class jsonResponseController9 extends Controller
{
    /*
    |-----------------------------------
    | Add Ledger Item
    ...
    public function loadAllItems(Request $request)
    {
        ...
        $all_bank_items = DB::table('ars_banks9')->where('match_flag', 0)->orderBy('date', 'ASC')->get();
        ...
        $all_ledger_items = DB::table('ars_ledgers9')->where('match_flag', 0)->orderBy('date', 'ASC')->get();
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank9::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank9::find($bank->id);
            ...
            $ledger = ArsLedger9::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger9::find($ledger->id);
    ...
    public function saveDataForSorting(Request $request)
    {
      ...
      $check_data = DB::table('tblReconSetup9')->get();
      if (count($check_data) > 0) {
          $proccess_update = DB::table('tblReconSetup9')->update([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
          ...
          $proccess_update = DB::table('tblReconSetup9')->insert([
              'BankName'   => $bank,
              'Location'   => $location,
              'LedgerName' => $ledger,
              'StartDate'  => $startDate,
