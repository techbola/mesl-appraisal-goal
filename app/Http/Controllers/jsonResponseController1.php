...
use Cavidel\ArsLedger1;
use Cavidel\ArsBank1;
use DB;

class jsonResponseController1 extends Controller
{
    /*
    |-----------------------------------
    | Add Ledger Item
    ...
    public function loadAllItems(Request $request)
    {
        ...
        $all_bank_items = DB::table('ars_banks1')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
        ...
        $all_ledger_items = DB::table('ars_ledgers1')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank1::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank1::find($bank->id);
            ...
            $ledger = ArsLedger1::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger1::find($ledger->id);
    ...
    public function saveDataForSorting(Request $request)
    {
        ...
        $check_data = DB::table('tblReconSetup1')->get();
        if (count($check_data) > 0) {
            $proccess_update = DB::table('tblReconSetup1')->update([
                'BankName'   => $bank,
                'Location'   => $location,
                'LedgerName' => $ledger,
                'StartDate'  => $startDate,
            ...
            $proccess_update = DB::table('tblReconSetup1')->insert([
                'BankName'   => $bank,
                'Location'   => $location,
                'LedgerName' => $ledger,
                'StartDate'  => $startDate,
