...
use Cavidel\ArsLedger2;
use Cavidel\ArsBank2;
use DB;

class jsonResponseController2 extends Controller
{
    /*
    |-----------------------------------
    | Add Ledger Item
    ...
    public function loadAllItems(Request $request)
    {
        ...
        $all_bank_items = DB::table('ars_banks2')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
        ...
        $all_ledger_items = DB::table('ars_ledgers2')->where('recon_flag', 0)->orderBy('date', 'ASC')->get();
    ...
    public function flagItem(Request $request)
    {
        ...
        if ($item_type == 'bank') {
            // update bank
            $bank = ArsBank2::where('id', $item_id)->first();
            if ($bank !== null) {
                ...
                $update_bank             = ArsBank2::find($bank->id);
            ...
            $ledger = ArsLedger2::where('id', $item_id)->first();
            if ($ledger !== null) {
                ...
                // find and update item
                $update_ledger             = ArsLedger2::find($ledger->id);
    ...
    public function saveDataForSorting(Request $request)
    {
        ...
        $check_data = DB::table('tblReconSetup2')->get();
        if (count($check_data) > 0) {
            $proccess_update = DB::table('tblReconSetup2')->update([
                'BankName'   => $bank,
                'Location'   => $location,
                'LedgerName' => $ledger,
                'StartDate'  => $startDate,
            ...
            $proccess_update = DB::table('tblReconSetup2')->insert([
                'BankName'   => $bank,
                'Location'   => $location,
                'LedgerName' => $ledger,
                'StartDate'  => $startDate,
