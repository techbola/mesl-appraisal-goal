    ...
    public static function allLeaveResumption()
    {
        ...
        if (count($all_leave_resumption_request) > 0) {
            ...
            foreach ($all_leave_resumption_request as $lr) {
                $super_data = Staff::where('StaffRef', $lr->supervisor_id)->first()->user;
                $staff_data = User::where('id', $lr->staff_id)->first();
                $department = Department::where('DepartmentRef', $lr->department_id)->first();

                $first_approver  = Staff::where('StaffRef', $lr->first_approver_id)->first();
                $second_approver = Staff::where('StaffRef', $lr->second_approver_id)->first();
                $third_approver  = Staff::where('StaffRef', $lr->third_approver_id)->first();
