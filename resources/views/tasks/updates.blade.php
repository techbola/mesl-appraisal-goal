<div class="card-box">
      ...
      @foreach ($task->updates as $chat)
        <div class="inbox-item m-r-10">
          ...
          <p class="inbox-item-author text-muted">
              ...
              @if($task->project->supervisor && $chat->staff->StaffRef == $task->project->supervisor->StaffRef)
  ...
  @can ('task-owner', $task)
    <form action="{{ route('save_taskupdate', $task->TaskRef) }}" method="post" class="m-t-20">
      {{ csrf_field() }}
