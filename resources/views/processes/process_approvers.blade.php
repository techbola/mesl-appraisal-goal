...
@section('content')
      ...
      <div style="padding: 30px">
         <ul class="nav nav-pills pull-right">
             <li><a style="background: #bbb" href="{{ route('ProcessManagement') }}">Return to Process Dashboard</a></li>
             <li><a href="#" style="color: #fff" data-target="#modalFillIn2" data-toggle="modal" id="btnFillSizeToggler2" class="btn btn-lg btn-info">Staff Process Staff Permissions</a></li>
         </ul>
      </div><div class="clearfix"></div>
  			<div class="card-title pull-left">List of Process Approvers</div><div class="clearfix"></div>
           <div class="row"><hr>
                    ...
                    <tr>
                      ...
                      @if($approve->Status == 0)
                         <a href="#" class="btn-default btn btn-sm disabled" title="">Disabled Staff</a>
                          @else
                          <a href="#" data-id="{{ $approve->ProcessApproverRef }}" data-target="#modalFillIn2" data-toggle="modal" id="change_modal"   class="btn-danger btn btn-sm">Change Status</a>
                          @endif
                      </td>
                    </tr>
                 @endforeach
            ...
            <div class="modal-dialog ">
              <div class="modal-content">
                ...
                <div class="modal-header">
                  <h5 class="text-left p-b-5"><span class="semi-bold" style="color: #000" id="title">New Process Approver</span></h5><hr>
                </div>
                <div class="modal-body">
                  <div class="row" >
                    ...
                    {{ Form::open(['id'=>'approver_form','autocomplete' => 'off', 'role' => 'form']) }}
                      ...
                      <div class="col-md-12">
                                <input type="submit" class="btn btn-sm btn-info pull-right" id="add_approver" data-dismiss="modal" value="Add New Process Approver">
                                <input type="submit" class="btn btn-sm btn-danger pull-right hide" id="change_process" data-dismiss="modal" value="Change Staff Status">
...
$(document).ready(function() {
  ...
  $("#btnFillSizeToggler2").click(function(e){
        $('#title').text('Add New Process Approver.');
         $('#change_process').addClass('hide');
  ...
  $(document).on('click', '#change_modal', function(event) {
        $('#title').text('Are you sure you want stop staff from creating company processes ?');
         $('#change_process').removeClass('hide');
 ...
 $("#add_approver").click(function(e){
    $.post('/store_process_approvers', $('#approver_form').serialize(), function(data, status) {
     console.log(status);
  ...
  $("#change_process").click(function(e){
    var id = $('#xyz').text();
    $.get('/change_process_approvers/'+id, function(data, status) {
     console.log(status);
     $('#policy_table').load(location.href + ' #policy_table');
    });
