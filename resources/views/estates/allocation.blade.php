...
@section('content')
  <div class="card-box">
    ...
    <form id="update_form" action="{{ route('update_allocation') }}" method="post">
      {{ csrf_field() }}
      {{ method_field('PATCH') }}
...
@push('scripts')
        ...
        data.forEach(function(unit){
          ...
          $('#unit_list tbody').append(`
            <tr>
              ...
              <td>
                <select class="form-control" name="Units[${unit.AllocationRef}]">
  ...
  <script>
    $('#update_form').on('submit', function(e) {
      e.preventDefault();
      $('#spinner').show();
      $.post('/update_allocation', $('#update_form').serialize(), function(data, status){
