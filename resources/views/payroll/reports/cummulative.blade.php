	...
	<div class="card-box">
		...
		<div class="panel-body">
			<table class="table tableWithSearch table-striped table-bordered">
				<thead>
					<th>Staff Name</th>
					<th>Cummulative Net Pay</th>
					<th>Cummulative Pension</th>
					<th>Cummulative PAYE</th>
				</thead>
				<tbody>
					@foreach($cummulatives as $cumm)
					<tr>
						<td>{{ $cumm->Fullname ?? 'No Name' }}</td>
						<td>{{ number_format($cumm->CummNetPay, 2) }}</td>
						<td>{{ number_format($cumm->CummNetPay, 2) }}</td>
						<td>{{ number_format($cumm->CummPension, 2) }}</td>
					</tr>
					@endforeach
				</tbody>
			</table>
