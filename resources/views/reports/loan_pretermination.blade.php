...
@section('content')
  ...
  <div class="panel panel-default" id="print-content">
    <div class="panel-heading">
      <h3 class="panel-title">Loan Pre-Termination</h3>
    ...
    <div class="panel-body">
      <table class="table tableWithExportOptions table-bordered">
        <thead>
          <tr>
            <th width="5%">GLRef</th>
            <th>Customer</th>
        ...
        <tbody>
          ...
          @foreach ($accounts as $account)
            <tr>
              ...
              <td>
                <a href="#" class="btn btn-sm btn-info" onclick="confirm2('Pre-terminate this loan?', '', 'form_{{ $account->GLRef }}')">Pre-Terminate</a>
                <form class="hidden" action="{{ route('preterminate', $account->GLRef) }}" method="post" id="form_{{ $account->GLRef }}">
                  {{ csrf_field() }}
                  <input type="hidden" name="amount" value="{{ $account->ClearedBalance * -1 }}">
                </form>
              </td>
            </tr>
          @endforeach
